...
import { solarite as solariteV1Address } from '../../../constants/tokenAddresses'
...
const MigrationInstructionsModal: React.FC<ModalProps> = ({ onDismiss }) => {
  ...
  const solariteTokenContract = useMemo(() => getContract(ethereum as provider, solariteV1Address), [])
  const migrationContract = solarite ? (solarite as any).contracts.solariteV2migration : undefined

  const allowance = useAllowance(solariteTokenContract, migrationContract)
  const { onApprove } = useApprove(solariteTokenContract, migrationContract)
