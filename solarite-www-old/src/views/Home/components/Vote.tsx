...
const Vote: React.FC<VoteProps> = () => {
  ...
  const handleVoteClick = useCallback(() => {
    delegate(solarite, account)
  }, [account, solarite])

  const fetchVotes = useCallback(async () => {
    const voteCount = await getVotes(solarite)
    setTotalVotes(voteCount)
  }, [solarite, setTotalVotes])
  ...
  const fetchDidDelegate = useCallback(async () => {
    const d = await didDelegate(solarite, account)
    if (d) {
      const amount = await getDelegatedBalance(solarite, account)
      setDelegatedBalance(amount)
    }
    setDelegated(d)
  }, [setDelegated, solarite, account, setDelegatedBalance])

  useEffect(() => {
    if (solarite && account) {
      fetchDidDelegate()
    }
  }, [fetchDidDelegate, solarite, account])
  ...
  return (
    <Card>
      <CardContent>
          ...
          <StyledCenter>
            ...
            <div style={{
              ...
              <div style={{
                  ...
                  marginTop: 4,
                  marginLeft: 4,
                }}>{`/ ${Number(new BigNumber(160000).multipliedBy(scalingFactor).toFixed(0)).toLocaleString()} SOLARITE`}</div>
            </div>
          </StyledCenter>
        </StyledResponsiveWrapper>
        <Spacer />
        ...
        <Spacer />
        {!delegated ? (
          <Button text="Delegate to save SOLARITE" onClick={handleVoteClick} />
        ) : (
          <div>
            <StyledDelegatedCount>Delegating: {Number(delegatedBalance.multipliedBy(scalingFactor).toFixed(0)).toLocaleString()} SOLARITE</StyledDelegatedCount>
        ...
        <div style={{
          ...
          <p>NOTE: You must harvest your SOLARITEs BEFORE 7am UTC Thursday 8/13 - very soon.</p>
          <p>Hold them in your wallet until 10PM UTC Sunday 8/16 for your delegation to save SOLARITE</p>
        </div>
          <div style={{
            display: 'flex',
            alignItems: 'center',
          ...
          <StyledLink target="__blank" href="https://twitter.com/SolariteFinance/status/1293660938906869760">More Info</StyledLink>
        </div>
      </CardContent>
    </Card>
  )
